...
function QuestionsPageById() {
  ...
  const fetchVote = async () => {
    ...
    axios
      .get(`http://localhost:8082/api/votes/status`, {
        params: {
          entityType,
          entityId,
          userId,
  ...
  const handleVote = (value) => {
    axios
      .post(`http://localhost:8082/api/votes/Question/${questionId}`, null, {
        params: {
          userId,
          value,
        },
  ...
  const handleVote1 = (value, id) => {
    axios
      .post(`http://localhost:8082/api/votes/Answer/${id}`, null, {
        params: {
          userId,
          value,
        },
  ...
  useEffect(() => {
    const fetchAnswers = async () => {
      try {
        const response = await axios.get(`http://localhost:8082/api/questions/${questionId}/answers`);
